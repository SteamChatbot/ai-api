...
import com.sanavi.aiapi.analysis.dto.ChecklistCheckUpdateDto;
...
public class AnalysisServiceImpl implements AnalysisService {
    ...
    private AnalysisAcceptedDto callFastApiRequest(AnalysisRequestDto request) {
        ...
        try {
            // Circuit Breaker가 OPEN이면 rawCallFastApiRequest()를 호출하지 않고 즉시
            // CallNotPermittedException 던짐
            ...
            throw new ResponseStatusException(HttpStatus.BAD_GATEWAY,
                    "AI 서버 오류: " + extractDetail(e.getResponseBodyAsString()));
    ...
    // recordExceptions: HttpServerErrorException, ResourceAccessException (FastAPI
    // 장애)
    ...
    public void saveChecklistChecks(
        ...
        boolean owned = analysisResultMapper.existsOwnedResult(taskId, userId);

        if (!owned) {
            throw new ResponseStatusException(
                    HttpStatus.NOT_FOUND,
                    "분석 결과를 찾을 수 없거나 권한이 없습니다.");
        }

        for (ChecklistCheckUpdateDto item : request.getItems()) {
            int affected = analysisResultMapper.updateChecklistChecked(
                    taskId,
                    item.getId(),
                    item.isChecked());
